...
def get_importances(model_path, dataset, max_tokens=150, max_samples=0, layer_type='mlp', metric='cos', write_json=True):
    '''
    dataset: List[str]
    '''
    ...
    results = write_tokens_to_file(model, dataset, tokenizer, generate_tokens=max_tokens)
...
def main():
    ...
    imps = get_importances(args.model_path, questions, args.max_tokens, args.max_samples, args.layer_type, args.metric, write_json=True)
